...
import json
import re
from typing import List, Optional, Tuple, Union, Self,Iterator,Dict,Any
import aiohttp
...
class URLParam(BaseModel):
    ...
    type:Literal['list','range'] = 'range'
    exclude:Optional['URLParam'] = None

    @model_validator(mode='after')
    def validate_range_or_list(self:Self)->Self:
            ...
            match self.type:
                ...
                case 'range':
                    ...
                    if self.values[0] > self.values[2]:
                        raise ValueError("For range, start must be less than end")
                    
                    if self.values[1] == 0:
                        raise ValueError("Step cannot be 0")
                    
                    range(self.values[0],self.values[2],self.values[1])
                        
        elif isinstance(self.values,list):
            if self.type == 'range' or self.type=='ranw':
    ...
    @model_validator(mode='after')
    def validate_exclude(self:Self)->Self:
        if self.exclude != None and (self.type == 'list' or self.exclude.type == 'list'):
            self.exclude = None
            return self
        ...
        if self.exclude != None:
            ...
            if self.values[0] > self.exclude.values[0]:
                raise ValueError("Exclude range must start after main range")
            
            if self.values[2] < self.exclude.values[2]:
                raise ValueError("Exclude range must end before main range")

            if self.values[1] > self.exclude.values[1]:
                raise ValueError("Exclude step must be greater than main step")
...
def subtract_stepped_ranges(self:URLParam,other:URLParam):
    if self.type == 'list' or other.type == 'list':
        return []
    s2, e2, step2 = other.values
    
    v = []
    for x in range(self.values[0],self.values[2],self.values[1]):
        in_bounds = (s2 <= x < e2) if step2 > 0 else (e2 < x <= s2)
        is_in_r2 = in_bounds and (x - s2) % step2 == 0
...
def generate_urls(config: URLConfigModel) -> Iterator[str]:
    def process_values(val: URLParam) -> List[Union[str, int, bool]]:
        if isinstance(val.values, tuple):
            if val.exclude:
                return subtract_stepped_ranges(val,val.exclude)
            start, step, end = val.values
            return list(range(start, end + 1, step))
        elif isinstance(val.values, list):
            return val.values
        else:
            return [val.values]

    # Process path and query parameters
    path_values = {k: process_values(v) for k, v in config.path_params.items()}
    query_values = {k: process_values() for k, v in config.query_params.items() if v.values}
