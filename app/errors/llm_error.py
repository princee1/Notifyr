...
class LLMModelNotPermittedError(BaseError):
    def __init__(self, provider:str,model:str|list[str],models:list[str]):
        super().__init__()
        self.provider = provider
        self.model = model
        self.models = models
...
class LLMModelMaxTokenExceededError(BaseError):
    def __init__(self,provider:str,agent_max_token:int,llm_max_token:int,mode:Literal['input','output']):
        super().__init__()
        self.provider = provider
