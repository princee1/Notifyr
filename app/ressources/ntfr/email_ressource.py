...
class EmailRessource(BaseHTTPRessource):
    ...
    if CAPABILITIES['object']:
        ...
        def get_template_schema(self,request:Request,response:Response,template:str='',authPermission=Depends(get_auth_permission), clientInfo:ClientTokenInfo = Depends(get_client_info),wait_timeout: int | float = Depends(wait_timeout_query)):
            print(template)
