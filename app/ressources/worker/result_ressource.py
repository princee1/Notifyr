...
from app.classes.auth_permission import AuthPermission, ClientAccessInfo, MustHave, Role
...
class CeleryResultRessource(BaseHTTPRessource):
    ...
    async def check_task(self,task_id:str,request:Request,authPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
        return await self.celeryService.seek_result(task_id)

    @UseHandler(CeleryTaskHandler)
    @BaseHTTPRessource.Delete('/task/{task_id}/')
    async def cancel_task(self,task_id:str,request:Request,authPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
        return await self.celeryService.cancel_task(task_id)

    @UseHandler(CeleryTaskHandler)
    @BaseHTTPRessource.Get('/schedule/{schedule_id}/{index}')
    async def check_schedule(self,schedule_id:str,index:int,request:Request,authPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
        return await self.celeryService.seek_schedule(schedule_id,index)
        
    @UseHandler(CeleryTaskHandler)
    @BaseHTTPRessource.Delete('/schedule/{schedule_id}/{index}')
    async def delete_schedule(self,schedule_id:str,index:int,request:Request,authPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
    ...
    async def purge_celery_task(self,queue:str, request:Request,task_id:str, authPermission:AuthPermission = Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
        """
...
class APSSchedulerRessource(BaseHTTPRessource):
    ...
    async def get_all_job(self,request:Request,response:Response,authPermission:AuthPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
    ...
    async def get_job(self,request_id:str,index:int,request:Request,response:Response,authPermission:AuthPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
    ...
    async def remove_all_jobs(self,request:Request,response:Response,authPermission:AuthPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
        jobs:list[Job] = await self.taskService.get_jobs()
        result = []
        for j in jobs:
            job_id:str = j.id
    ...
    async def remove_job(self,request_id:str,index:int,request:Request,response:Response,authPermission:AuthPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
        job_id = APSchedulerConstant.REDIS_APS_ID_RESOLVER(request_id,index)
    ...
    async def pause_job(self,request_id:str,index:int,request:Request,response:Response,authPermission:AuthPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
        job_id = APSchedulerConstant.REDIS_APS_ID_RESOLVER(request_id,index)
    ...
    async def resume_job(self,request_id:str,index:int,request:Request,response:Response,authPermission:AuthPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
        job_id = APSchedulerConstant.REDIS_APS_ID_RESOLVER(request_id,index)
...
class ResultBackendRessource(BaseHTTPRessource):
    ...
    def get_result(self,request:Request,authPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
        return 
    

    @BaseHTTPRessource.Get('/permission/{ws_path}',)
    def invoke_notify_permission(self, ws_path:str,request:Request, authPermission=Depends(get_auth_permission), clientInfo:ClientAccessInfo = Depends(get_client_info)):
