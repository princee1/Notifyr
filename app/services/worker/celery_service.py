...
class ChannelMiniService(BaseMiniService):
    ...
    async def pingService(self,infinite_wait:bool,data:dict,profile:str=None,as_manager:bool=False,**kwargs):
        ...
        if self.configService.CELERY_BROKER_PROVIDER == 'rabbitmq':
...
class CeleryService(BaseMiniServiceManager[ChannelMiniService], IntervalInterface):
    ...
    def verify_dependency(self):
        ...
        if self.configService.CELERY_BROKER_PROVIDER not in ['rabbitmq','redis']:
            raise BuildOkError('')
        
        if self.configService.CELERY_BROKER_PROVIDER == 'redis' and self.redisService.service_status == ServiceStatus.NOT_AVAILABLE:
            raise BuildOkError('')

        if self.configService.CELERY_BROKER_PROVIDER == 'rabbitmq' and self.rabbitmqService.service_status == ServiceStatus.NOT_AVAILABLE:
            raise BuildOkError('')
